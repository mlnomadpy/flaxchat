...
def main(argv: list[str] | None = None) -> int:
    ...
    command = shlex.join(spec.argv)  # tpuz transport boundary; spec remains structured.
    if args.profile:
    ...
    if args.run_once:
        vm.run_once(command, sync=".", collect_files=args.collect, gcs=gcs, notify_url=args.notify)
        return 0
    ...
    try:
        ...
        if gcs:
            vm.run_with_resume(command, gcs=gcs, run_name=args.run_name, sync=".", secrets=args.secrets)
        else:
